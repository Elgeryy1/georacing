...
1. **Heartbeat (cada 10s):** La beacon envía su estado actual al servidor
2. **Polling (cada 300ms):** La beacon consulta:
...
**Propósito:** Define la interfaz visual completa de la beacon.
...
| `_beaconId` | string | ID único de la beacon |
...
**Propósito:** Gestiona la configuración local de la beacon.
...
| Description | string | Descripción de la beacon |
...
| `SendHeartbeatAsync()` | POST /beacons/heartbeat | Envía estado de la beacon |
| `GetPendingCommandsAsync()` | GET /commands/pending/{uid} | Obtiene comandos pendientes |
| `GetBeaconConfigAsync()` | POST /_get | Obtiene config de la beacon |
...
| IsConfigured | bool | Si la beacon está configurada |
...
| Método | Descripción |
|--------|-------------|
| `SyncConfigAsync()` | Lee configuración de la beacon desde BD |
...
| `/beacons/heartbeat` | POST | Registrar estado de beacon |
