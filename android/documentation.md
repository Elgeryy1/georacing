...
- Detección BLE de beacons para determinar la zona actual del usuario.
...
| Librería | Justificación |
|---|---|
| **Android Bluetooth LE API** | API nativa de Android para escanear y emitir señales BLE. Las beacons físicas del circuito emiten BLE y la app lo detecta para saber la zona del usuario. |
...
- `beaconsRepository` — Repositorio de beacons (OfflineFirst).
...
- `BeaconsRepository` — Gestión de beacons BLE del circuito.
...
- **`BeaconAdvertiser.kt`** — Emite señales BLE desde el teléfono (modo staff para que las beacons sepan que el personal está cerca).
- **`BlePayloadParser.kt`** — Parser del payload BLE propietario.
- **`BleCircuitSignal.kt`** — Modelo del dato BLE parseado.
- **`DetectedBeacon.kt`** — Modelo de una beacon detectada (incluye RSSI para calcular distancia).
...
- **`dto/`** — Data Transfer Objects para la serialización JSON:
  ...
  - `BeaconConfigDto` — Configuración de beacon.
...
- **`dao/`**:
  - `BeaconDao` — Operaciones CRUD sobre beacons.
...
- `OfflineFirstBeaconsRepository` — Beacons: Room caché + sincronización con API.
...
2. **BLE** — `BeaconScanner` detecta las señales de las beacons físicas cercanas.
...
En modo staff, el teléfono puede emitir señales BLE para identificarse como personal del evento. Esto permite a las beacons físicas saber que hay staff cerca.
