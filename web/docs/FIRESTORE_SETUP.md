...
Cada documento representa una beacon física. El ID del documento debe coincidir con el `beaconId` de la beacon.
...
```javascript
{
  beaconId: "BEACON-GRADA-G-ENTRADA",
  zone: "GRADA-G",
  mode: "NORMAL",
  arrow: "RIGHT",
...
|-------|------|---------|-------------|
| `beaconId` | string | - | ID único de la beacon |
...
| `lastSeen` | timestamp | - | Último heartbeat de la beacon |
...
### Para Beacons (BeaconDisplay en Windows)
...
- ✅ **Leer** todas las beacons
- ✅ **Crear** nuevas beacons
- ✅ **Actualizar** cualquier campo de cualquier beacon
- ✅ **Eliminar** beacons
...
const beaconRef = doc(db, "beacons", "BEACON-GRADA-G-ENTRADA");
...
La beacon debe actualizar su estado cada 30-60 segundos:
...
const beacons = [
  ...
  {
    beaconId: "BEACON-GRADA-A-SECTOR-1",
    zone: "GRADA-A",
    mode: "NORMAL",
    arrow: "UP",
...
### Panel de Control
- Dashboard muestra estado online/offline de cada beacon
- Última conexión (`lastSeen`)
- Modo actual

### Detección de Beacons Offline

Las beacons que no envían heartbeat en más de 2 minutos deben marcarse como offline. Esto puede hacerse con:
